...
void Player::Update(std::vector<Part> parts, int partCount) {
	...
	for (int i = 0; i < partCount - 5; i++) {
	  int sphereSize = 8;
	  bool canMoveX = !parts[i].isObstructed(-(pos_x + (_xx*8)), -pos_y, -pos_z, 16*sphereSize);
	  bool canMoveZ = !parts[i].isObstructed(-pos_x, -pos_y, -(pos_z + (_zz*8)), 16*sphereSize);
	  bool canMovePosY = !parts[i].isObstructed(-pos_x, -(pos_y + 1024 + 512 + walkVelocityRelative_y), -pos_z, abs(walkVelocityRelative_y * 4)*sphereSize);
	  bool canMoveNegY = parts[i].isObstructed(-pos_x, -(pos_y - 1024 - 512 + walkVelocityRelative_y), -pos_z, abs(walkVelocityRelative_y * 4)*sphereSize);

	  if (canMoveX) pos_x = targetPoint_x;
	  if (canMoveZ) pos_z = targetPoint_z;
